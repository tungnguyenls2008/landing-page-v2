...
/*!******************************************************!*\
  !*** ./resources/metronic/js/pages/layout/skills.js ***!
  \******************************************************/
 // Class definition
...
var KTDatatableHtmlTableDemo = function () {
  ...
  var demo = function demo() {
    var datatable = $('#kt_datatable_skills').KTDatatable({
      data: {
        saveState: {
          cookie: false
        }
    ...
    $('#kt_datatable_skills').on("click", ".delete", function (e) {
      e.preventDefault();
      var id = $(this).data('id');
      Swal.fire({
        ...
        if (result.value) {
          ...
          var sendUri = HOST_URL + "/layout/skills/" + id;
          $.ajax({
            url: sendUri,
            method: 'DELETE',
    ...
    $('#kt_datatable_skills').on("click", ".edit", function (e) {
      e.preventDefault();
      var id = $(this).data('id');
      $(".page-loader").addClass("bg-dark-o-50");
      $("#kt_body").addClass("page-loading");
      ...
      $.ajax({
        ...
        complete: function complete(xhr, status) {
          if (xhr.status === 200 && xhr.responseJSON.data) {
            ...
            $("#editModal [name=title]").val(data.title);
            $("#editModal [name=title_ar]").val(data.title_ar);
            $("#editModal [name=amount]").val(data.amount);
